                                                        ...
                                                        <tr v-for="row in rows">
                                                            ...
                                                            <td>
                                                                <select v-model="row.kategori" class="bg-light custom-select">
                                                                    ...
                                                                    <option value="Deging">Deging</option>
                                                                    ...
                                                                    <option value="Tabung Gas">Tabung Gas</option>
                                                                </select>
                                                            </td>
                                                            <td>
...
<script>
    new Vue({
        ...
        methods: {
            ...
            addRow: function () {
                this.rows.push({id: null,nama_produk: '', kategori: ''});
            },
            deleteRow: function () {
                this.rows.pop();
            },
            ...
            getProduct(id){
                axios(`/api/getproduct/${id}`)
                .then((res)=>{
                    const {data} = res
                    for(let i=0 ; i<data.length;i++){
                        this.rows.push({id: data[i].id,nama_produk: data[i].nama_produk, kategori: data[i].kategori })
                    }
                    this.getProvinsi()
                })
            },
            getData(){
                ...
                axios.get(`/getdatapedagang/${id}`)
                .then((res)=>{
                    const {data} = res;
                    this.id = data.id
                    this.nama= data.nama;
                    this.alamat_rinci= data.alamat_rinci;
                    this.no_hp= data.no_hp;
                    this.no_wa= data.no_wa;
                    this.foto_pedagang= data.foto_pedagang;
